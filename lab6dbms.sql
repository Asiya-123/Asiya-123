...
select * from invoice_master natural join invoice_item natural join product natural join customer where inv_date='2021-02-03';
select cust_name,pname,inv_date from customer natural join product natural join invoice_master natural join invoice_item where pname="Soap";
...
select * from customer where phn_no is null and city="Ranni";
select pname from product where p_id in(select p_id from invoice_item where quantity=5);
